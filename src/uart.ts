...
import { Deferred, AsyncQueue } from './utils';
...
export class UartProtocol implements AsyncIterable<Buffer> {
    ...
    constructor(
        private writeCb: (data: Buffer) => Promise<void>) {
                    ...
                    if ((data[0] === 192)) {
                        ...
                        if ((data[0] === 193)) {
                            ...
                            if ((data[0] === 194)) {
                                ...
                                log.error("UNKNOWN FRAME RECEIVED: %r", data);
                            }
                        }
                    }
                }
    ...
    data_frame_received(data: Buffer) {
        ...
        var seq;
        log.debug("Data frame: %s", data.toString('hex'));
        seq = ((data[0] & 112) >> 4);
    ...
    rstack_frame_received(data: Buffer) {
        ...
        log.debug("RSTACK Version: %d Reason: %s frame: %s", data[1], code.toString(), data.toString('hex'));
        if (NcpResetCode[<any>code].toString() !== NcpResetCode.RESET_SOFTWARE.toString()) {
            return;
        }
        if ((!this._reset_deferred)) {
            log.warn("Reset future is None");
            return;
        }
    ...
    error_frame_received(data: Buffer) {
        ...
        log.debug("Error frame:", data.toString('hex'));
    }

    write(data: Buffer) {
        /* Send data to the uart */
        log.debug("Sending:", data.toString('hex'));
